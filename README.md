...
## Scalability
The project allows for scaling by:
- **Batching classification**: instead of classifying one-by-one, multiple files are passed through the models at the same time. 
- **PostreSQL**: this database system is capable of handling concurrent connections and large volumes of data.
- **Docker**: by containerizing, the project can be easily deployed on more or larger systems. Additionally, because the database has its own container, this can be scaled separately.

Possible extensions include: 
- GPU acceleration: to ensure a quick start-up (no 10 min CUDA torch download), the project runs fully on CPU. However, to enable larger models and faster inference times, GPU can be easily integrated into the project.
- Parallel processing: ingesting multiple files concurrently can help speed up the system. However, for smaller volumes of data, this is not necessary.
...
```
## Running the app
You can spin up the app container by running:
```bash
docker compose up
```
